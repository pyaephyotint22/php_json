		...
		<div class="row mt-5">
			<div class="col align-self-center">
				...
				<div class="form-group row">
					...
					<div class="col-sm-10">
						<input type="text" class="form-control" id="edit_email" placeholder="Enter Email" name="edit_email">
					</div>
				</div>
				<fieldset class="form-group">
					<div class="row">
						...
						<div class="col-sm-10">
							...
							<div class="form-check">
								...
								<label class="form-check-label" for="edit_male">Male
								</label>
							</div>
							<div class="form-check">
								<input class="form-check-input" type="radio" name="edit_gender" id="edit_female" value="Female">
								<label class="form-check-label" for="edit_female">Female
								</label>
							</div>
						</div>
					</div>
			...
			$.get('student.json',function (response) {
				if (response) {
					...
					$.each(stuArray,function(i,v){
						...
						<td>${v.gender}</td>
						<td>${v.email}</td>
						<td><button class="btn-primary btn-sm detail" data-id="${i}" data-name="${v.name}"
						 data-gender="${v.gender}"
						data-email="${v.email}" data-address="${v.address}" data-profile="${v.profile}">Detail</button>
						<button class="btn-success btn-sm delete" data-id="${i}" >Delete</button>
						<button class="btn-warning btn-sm edit" data-id="${i}">Edit</button>
						</td></tr>`;
					});
					$("tbody").html(html);
				}
